...
void open_process(t_dword pid)
{
	...
	if (is_error()) error("OpenProcess Error");
}
...
void get_moduel_info(const char* name, t_dword& address, t_dword& size)
{
	...
	if (is_error()) error("GetModuleInfo error");

	MODULEENTRY32 mod{ sizeof(MODULEENTRY32) };
	BOOL status = Module32First(snap, &mod);
	while (status)
	{
		if (strncmp(name, (char *)mod.szModule, strlen(name)) == 0)
		{
...
int main(int argc, char* argv[])
{
	...
	get_moduel_info("client.dll", g_client, g_client_size);
